...
public class MySQLTableColumn extends JDBCTableColumn<MySQLTableBase> implements DBSTableColumn, DBSTypedObjectExt3, DBPNamedObject2, DBPOrderedObject
{
    ...
    private void loadInfo(ResultSet dbResult)
        throws DBException
    {
        if(MySQLUtils.isMyCat(super.getTable().getDataSource().getInfo())){
            loadMyCatInfo(dbResult);
        }else{
            loadMySqlInfo(dbResult);
        }
    }

    private void loadMySqlInfo(ResultSet dbResult) throws DBException{
    ...
    private static final Pattern TYPE_NAME_P = Pattern.compile("^(\\w+)(?:\\((\\d+)(?:,(\\d+))?\\))?");
    private void loadMyCatInfo(ResultSet dbResult) throws DBException{
        name = JDBCUtils.safeGetString(dbResult, "Field");
        ordinalPosition = 0;
        this.fullTypeName = JDBCUtils.safeGetString(dbResult, "Type");
        Matcher m = TYPE_NAME_P.matcher(this.fullTypeName);
        m.find();
        String typeName = m.group(1);
        assert typeName != null;
        String keyTypeName = JDBCUtils.safeGetString(dbResult, "Key");
        if (!CommonUtils.isEmpty(keyTypeName)) {
            try {
                keyType = KeyType.valueOf(keyTypeName);
            } catch (IllegalArgumentException e) {
        ...
        setValueType(MySQLUtils.typeNameToValueType(typeName));
        DBSDataType dataType = getDataSource().getLocalDataType(typeName);
        if(typeName.contains("char") && m.group(2) != null && !m.group(2).isEmpty()){
            this.charLength = Long.parseLong(m.group(2));
        }
        if (this.charLength <= 0) {
            if (dataType != null) {
                setMaxLength(CommonUtils.toInt(dataType.getPrecision()));
        ...
        this.required = !"YES".equals(JDBCUtils.safeGetString(dbResult, "Null"));
        if(!typeName.contains("char")){
            if(m.group(2) != null && !m.group(2).isEmpty()){
                this.setPrecision(Integer.valueOf(m.group(2)));
            }
            if(m.group(3) != null && !m.group(3).isEmpty()){
                this.setScale(Integer.valueOf(m.group(3)));
            }
        }
        ...
        this.collation = getDataSource().getCollation(JDBCUtils.safeGetString(dbResult, "Collation"));
